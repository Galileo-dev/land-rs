...
- *Landing Accuracy*: @ppo can achieve near-pinpoint landings if the reward strongly penalises position errors. In a 3-DOF simulation task, a @ppo policy can bring the vehicle on the pad within a few meters. This shows that @ppo alone is insufficient to match the performance of sub-meter accuracy of a finely tuned classical controller.

- *Fuel Efficiency*: A minimisation fuel reward can be incorporated into a @ppo policy, but reaching an actual fuel-optimal landing similar to @gfold is challenging. It was found that a 6-DOF @ppo agent uses 60% of the fuel used by @sc @IAFRATE202540. Suggesting a well-trained RL agent can significantly outperform traditional methods in fuel efficiency.

- *Handling Disturbances*: During training, random disturbances (wind gusts, noisy sensors, centre-of-mass shifts, etc) were injected into the environment to help the agent generalise to real-world scenarios. The controller achieved low position and velocity errors in experiments under different unmodeled dynamics and disturbances. Almost matching that of a disturbance-free scenario. @ppo:pl closed-loop policy can react to continuous state deviations, providing the disturbance rejection of a classical feedback controller. However, an RL policy could be unstable and fail catastrophically if the disturbance/condition is far outside its training. Training with a broad set of scenarios is crucial, or domain randomisation is needed to achieve a robust policy.

- *Computational Complexity*: @rl has minimal runtime complexity, where the inference is a neural network forward pass. A real-world small network of about a few hundred neurons with an optional @lstm @Xue2023: running such a network on flight hardware is high-speed (the Imitation Learning + PPO-LSTM policy ran at 2.5 ms on a Jetson TX2 @Xue2023). Another group achieved inference of 10ms on a ZU9E embedded platform (Zynq UltraScale+ ZU9EG). The main trade-off with @rl is the offline training time and cost, which require significant compute resources, used to simulate many landing scenarios often on GPU clusters @IAFRATE202540. Once a @ppo policy is trained, it is a small, fixed function that can be evaluated on board.
...
== Conclusion
Landing an autonomous rocket is a complex task that requires both optimal control and machine learning. State-of-the-art @drl methods have been able to demonstate that successful and reliable landing with pinpoint accuarcy and high efficiency, rivaling the performance of classical methods like @gfold. Using imitation learning, @ppo and @sac have been able to learn effective descent guidance from expert trajectories. More importantly, @drl has shown great runtime performance and can run on embedded platforms, respond to disturbances, and simplify the overall control architecture by combining guidance and control decisions into a single neural network
