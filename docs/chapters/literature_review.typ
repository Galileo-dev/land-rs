= Literature Review
This chapter provides an overview of the existing methods for trajectory generation, both traditional techniques like polynomial guidance and @zemzev, and modern approaches including convex optimisation (G-FOLD) and next-generation Deep Reinforcement Learning approaches (@ppo, @sac)
...
Historically, on-board computational power was a significant factor that dictated which algorithms could be used for landing, but with modern hardware and modern guidance, computer architecture like the Spacex Falcon 9 or Starship with its triplex redundant system.
...
Precision landing enables a critical turning point in space exploration and payload delivery costs. @rlv:pl significantly reduce the operational and refurbishment costs, and significantly improve the economic viability of space flight. Reusability focuses on reducing the costs associated with the vehicle's recovery, inspection and refurbishment, mainly with high wear components like engine, avionics and tanks. Precision landing significantly drives down turnaround time as the logistics of recovery and refurbishment are significantly reduced or eliminated @webb2016economics.
